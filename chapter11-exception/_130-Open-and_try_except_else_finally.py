...
try:
    ...
    第一个路径的位置可以使用 
    <1>  r"f:\123.txt"代表原生的字符串,忽略所有转义字符。
    <2>  "F:\\folder1\123.txt"代表非原生的字符串,其中盘符后面的目录需要用\\ 1个转义字符和一个反斜杠,看起来就像是两个反斜杠来表示。
    注：如果你写了详细的路径,它会去寻找对应的文件。如果你只写"123.txt"那么它会在当前的运行代码的目录下找文件,如果文件不存在则会生成并写入.
    '''
