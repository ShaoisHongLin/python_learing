...
os.listdir(path)         返回目录下的文件
    !!!!!注意:listdir会将当前目录的文件夹中的文件名,返回为一个列表。
    !!!!!注意:listdir只会返回当前目录内的child而不会一层层地打开存入返回到列表,仅仅获取子级的内容返回到列表。
    ['subdir1', 'subdir2', 'file4.txt']
    列表的每个元素是字符串。对于字符串在python中又是序列的一种,可以使用字符串的方法来进行一些逻辑处理。
    可以利用字符串的逻辑查看后缀,"后缀是否存在"与"后缀是否符合格式"可能是经常用于筛选我们想要的内容的手段。
'''
