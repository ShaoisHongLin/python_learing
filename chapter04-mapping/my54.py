# 字典的基础知识和创建
# 1.字典是键值对的"无序"、"可变"序列，以键值对(key-value)存在。

# 2.序列中，list使用"下标数字"找到对象，dict使用"键(key)"找到对象

# 3.dict是可变序列对象其中之一。

# 4.键是不可变对象，不可以用set、dict、list这三个可变对象来作为键。。

# 5.键是不能重复的，如果你误写了两个重复的键，那么‘后’将覆盖‘前’，如：a={'name':"shao",'age':18,'score':90,"age":20}
# 那么覆盖后打印出来就是20

# 字典的创建：有4种，普通、dict()函数、zip()配对函数将键值列表配对、dict.fromkeys()创建有键空值的字典。
